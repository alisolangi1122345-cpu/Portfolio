...
(function () {
  ...
  /* ---------------------------------------------------
  1. Scroll reveal — repeat animation on every scroll
  --------------------------------------------------- */
  function initScrollReveal() {
    const revealEls = document.querySelectorAll(".reveal-up");
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // Remove class so animation can happen again
...
if (educationTimeline) {
  ...
  function initHeroCanvas() {
    ...
    const NODE_COUNT_DIVISOR = 22000;
    ...
    function resize() {
      ...
      const count = Math.max(
        18,
        Math.floor(
          (canvas.offsetWidth * canvas.offsetHeight) / NODE_COUNT_DIVISOR,
        ),
      );
    ...
    function frame() {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < MAX_DIST) {
            ctx.strokeStyle = `rgba(59, 125, 255, ${
              0.12 * (1 - dist / MAX_DIST)
            })`;
